...
function drawMapForStopId(mapContainerId, agency, stopId, direction) {
    ...
    function drawRailLines() {
        var northLineColor = colorbrewer.Reds[5][1];
        var eastLineColor = colorbrewer.Blues[5][1];
        var lineWidth = 10;

		drawRoute("8747_0", northLineColor, lineWidth, false); // RED
		drawRoute("8746_0", northLineColor, lineWidth, false); // GOLD
		drawRoute("8766_0", eastLineColor, lineWidth, false); // BLUE
		drawRoute("8748_0", eastLineColor, lineWidth, false); // GREEN
    }
    ...
    function drawStations() {
        ...
        var globalConnSourceName = 'global-connected-stations';
        map.addSource(globalStationLocs, new mapboxgl.GeoJSONSource({
            data: { 
                "type": "MultiPoint",
                "coordinates": stationCoords
            },
        }));
        map.addSource(globalSourceName, {
            "type": "geojson",
            "data": {
        ...
        map.addSource(globalConnSourceName, {
            "type": "geojson",
            "data": {
                "type": "FeatureCollection",
                "features": connectingStationFeatures
            }
        });
	...
	function findBounds(allPoints) {
		...
		for(var i=0; i<allPoints.length; i++) {
			var lng = parseFloat(allPoints[i][0]);
			var lat = parseFloat(allPoints[i][1]);
		...
		return [
			[minLng-0.005, minLat-0.008], [maxLng+0.015, maxLat+0.01]
		];
	}
	...
	function drawStop(stopid_str) {
        ...
        switch(direction) {
            ...
            case "NE": angle = 315; break;
            case "NW": angle = 225; break;
        ...
        if (angle != undefined) {
            map.addLayer({
                "id": sourcename+'_direction',
                "type": "symbol",
                "source": sourcename,
                "layout": {
                    "text-field": String.fromCharCode(10148),
                    "text-font": ["Open Sans Semibold", "Arial Unicode MS Bold"],
                    "text-offset": [0, 0],
                    "text-anchor": "center",
                    "text-size": 25,
    ...
    function drawNonStationTerminus(routeids) {
        ...
        for (var k = 0; k < stopids.length; k++) {
            var stopid_str = stopids[k].stopid;
            var s = BUSDATA[agency].stop_data[stopid_str];
            ...
            terminusFeatures.push({
                    ...
                    "properties": {
                        "title": stopids[k].name
                    }
            });
        }

        var sourcename = 'terminus-location';
        map.addSource(sourcename, {
            "type": "geojson",
            "data": {
                "type": "FeatureCollection",
                "features": terminusFeatures
            }
        });
